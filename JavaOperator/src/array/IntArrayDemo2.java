...
public class IntArrayDemo2 {
	public static void main(String[] args) {
		/*
		 ...
		 2. 배열의 크기 = 배열의 요소의 갯수
		 3. 배열은 크기가 고정되어 있으면 요소를 추가할 수 없다.
		 4. 배열은 한가지 타입만 지정할 수 있다.
		 * */
		int[] arr= new int[3] ; // 3은 요소의 갯수 = 배열의 크기
		//몇칸 짜리 배열인가를 알려주는 것. size []안에 숫자값을 넣는 것이 초기화 이다.
		arr[0] = 1; //0부터 시작 (첫번째 칸) 첫번째칸에 숫자 1을 담는다.
		arr[1] = 2; // 배열 두번째칸에 숫자 2를 담는다
		arr[2] = 3; // 배열 세번째칸에 숫자 3을 담는다
		//arr[3] = 4; // 이것은 지금은 에러표시를 하지 않지만 프로그램을 실행시키면 에러가 발생한다.
		for (int i = 0; i < arr.length; i++) {
			// arr은 바로 직전에 선언한 객체[배열]이름
			// 배열객체.length는 배열의 크기입니다.
			// for문은 배열의 크기만큼만 looping한다.
